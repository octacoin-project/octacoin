...
#if defined(NDEBUG)
# error "Octacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x58145fb2a47f8accab0a4405f482d7f334157f2065cadf24ab397adc42716a9a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Octacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Octacoin Signed Message:\n";
...
{
    // Octacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
     ...
     if(nHeight == 1)
     {
        nSubsidy = 22100000 * COIN;
     }

    // Subsidy is cut in half every 221000 blocks
    nSubsidy >>= (nHeight / 221000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Octacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Octacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x58145fb2a47f8accab0a4405f482d7f334157f2065cadf24ab397adc42716a9a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1431682084, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "15/May/2015 Get Ready To Mine April 15th 2015 Octacoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x135127cdee0678f59899a2a92c602af3e53f0a28337f23e23afe756680aa15eb"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Octacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OctacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OctacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OctacoinMiner(CWallet *pwallet)
{
    printf("OctacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("octacoin-miner");
    ...
    try { loop {
        ...
        printf("Running OctacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OctacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OctacoinMiner, pwallet));
}
